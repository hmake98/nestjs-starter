...
describe('AwsSNSService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [AwsSNSService, { provide: ConfigService, useValue: configServiceMock }],
    }).compile();

    service = module.get<AwsSNSService>(AwsSNSService);
    snsClientMock = service['snsClient'] as unknown as jest.Mocked<SNSClient>;
  });
  ...
  describe('sendSms', () => {
    ...
    it('should send SMS successfully', async () => {
      ...
      snsClientMock.send = jest.fn().mockResolvedValue({ MessageId: mockMessageId });
    ...
    it('should throw an error if SMS sending fails', async () => {
      ...
      await expect(service.sendSms(phoneNumber, message)).rejects.toThrow('SMS sending failed');
    });

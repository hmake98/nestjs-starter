...
describe('AwsSESService', () => {
  ...
  describe('getTemplate', () => {
    ...
    it('should throw error if AWS SDK throws', async () => {
      mockSesClient.send.mockRejectedValue(new Error('AWS Error') as never);

      await expect(service.getTemplate({ name: 'Template1' })).rejects.toThrow('AWS Error');
    });
  });
  ...
  describe('deleteTemplate', () => {
    it('should delete a template', async () => {
      ...
      const result = await service.deleteTemplate({ name: 'TemplateToDelete' });
    ...
    it('should throw error if AWS SDK throws', async () => {
      ...
      await expect(service.deleteTemplate({ name: 'TemplateToDelete' })).rejects.toThrow(
        'AWS Error',
      );
    });
  });

...
import { AwsSESGetTemplate, AwsSESSend, AwsSESTemplate } from 'src/common/helper/dtos/aws.ses.dto';
...
export class AwsSESService implements IAwsSESService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.sesClient = new SESClient({
      credentials: {
        ...
        secretAccessKey: this.configService.get<string>('aws.ses.credential.secret'),
      },
      region: this.configService.get<string>('aws.ses.region'),
    });
  }

  async getTemplate({ name }: AwsSESGetTemplate): Promise<GetTemplateCommandOutput> {
    const command: GetTemplateCommand = new GetTemplateCommand({
      TemplateName: name,
    });

    try {
      const output = await this.sesClient.send<GetTemplateCommandInput, GetTemplateCommandOutput>(
        command,
      );
  ...
  async createTemplate({
    ...
    htmlBody,
    plainTextBody,
  }: AwsSESTemplate): Promise<CreateTemplateCommandOutput> {
  ...
  async deleteTemplate({ name }: AwsSESGetTemplate): Promise<DeleteTemplateCommandOutput> {
  ...
  async send<T>({
    ...
    templateName,
    templateData,
  }: AwsSESSend<T>): Promise<SendTemplatedEmailCommandOutput> {
